...
def test_moneylender_normal(turn: Turn, player: Player, game: Game, monkeypatch):
...
def test_moneylender_input_no(turn: Turn, player: Player, game: Game, monkeypatch):
    player.hand.add(moneylender)
    player.hand.add(copper)
    assert len(player.hand) == 2
    assert len(game.trash) == 0
...
def test_moneylender_no_coppers(turn: Turn, player: Player, game: Game, monkeypatch):
    player.hand.add(moneylender)
    assert len(player.hand) == 1
    assert len(game.trash) == 0
    player.hand.cards[0].play(turn, player, game)
